...
try:
    ...
    if args.apptype:
        ...
        try:
            dz = LINE(idOrToken, appType=args.apptype, systemName=args.systemname, channelId=args.channelid, showQr=args.showqr)
            tokenFile.close()
            tokenFile = tokenPath.open('w+')
            tokenFile.write(dz.authToken)
            tokenFile.close()
        ...
        for appType in listAppType:
            ...
            idOrToken = authToken if authToken else print("# No one Qr was readed, Lets Scan New QR.")
            try:
                dz = LINE(idOrToken, appType=appType, systemName=args.systemname, channelId=args.channelid, showQr=args.showqr)
                tokenFile.close()
                tokenFile = tokenPath.open('w+')
                tokenFile.write(dz.authToken)
                tokenFile.close()
...
def runningProgram():
    if Setmain['restartPoint'] is not None:
        try:
            dz.sendMessage(settings['restartPoint'], 'Aye Sir, Im Back!')
